...
-- 1. 도메인 무결성 제약조건
--		도메인 제약 (Domain Constraint)이로고도 하며,
...
--					고유하게 식별이 가능한 단위
--		아무리 많은 개체가 있더라도, 내가 원하는 '유일한' 개체를 발견할 수 있어야
--		테이블은 기본키를 지정하고, 절대로 NULL값을 가져서는 안되며,
...
-- 2. Unique : 단어 뜻 그대로 해당 칼럼에 들어가는 값이
...
create table example3(
	...
	col3 varchar2(10 char) unique constraint col3_unique unique(col3)
	col2 varchar2
);
...
-- 		참조받는 테이블은 참조하는 테이블의 PK나 Unique로 지정된 컬럼만을 
--							FK로 지정할 수 있음
--		외래키 설정을 위해서는 참조받는 컬럼이 먼저 생성되어야 하고,
--		그 이후에 외래키를 심을 테이블이 그 이후에 생성이 되어야함
...
create table 테이블명(
 	...
 	constraint FK(제약조건)명 foreign key(컬럼명)
 		...
 		on delete set null || on delete cascade -- 둘중에 하나를 골라서 입력
 		-- 참조받는 테이블에서 행 부분을 지웠을 때
 		-- 이 테이블에 연결되어 있는 행에 대해서
 		-- 그 데이터를 Null값을 먹일지 or 그 행도 같이 지울지에 대한 옵션
);
...
drop table subject cascade constraint purge;
drop table student cascade constraint purge;

create table subject(
	sub_id number (4) primary key,
	sub_name varchar2 (10 char) not null
);

insert into subject values(3131, '체육학과');
insert into subject values(3132, '법학과');
select * from subject;
--------------------------------------------------
...
insert into student values(101, '마석도', 3131);
insert into student values(102, '정인', 3132);
insert into student values(103, '신경원', 3131);
insert into student values(104, '남궁상', 3132);
insert into student values(105, '박진상', 3100); -- subject 테이블에는 3100 코드가 없으므로 오류 !

select * from student;

-- 어떤 특정학과가 없어지면 이 학생의 정보도 지워지게
delete from subject where sub_id = 3131;
-- subject테이블에서 3131코드를 지우면, student테이블에서 3131코드를 가지고 있던
--	모든 학생의 정보가 지워짐
---------------------------------------------------------------------
