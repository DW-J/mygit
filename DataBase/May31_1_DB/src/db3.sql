...
--alter table may31_coffee add c_taste varchar2(20 char) not null; -- 얘는 안됨!
...
-- 테이블 자체를 지우고(삭제) 싶은 경우 (휴지통에 넣기)
drop table [테이블명]; -- 요거는 될때도 있고 안될때도 있다.// 제약조건이나 외래키가 관련되어 있을 경우
drop table [테이블명] cascade constraint; -- 위에꺼 알빠아님 다 지울거시다 ~ !
...
------------------------------------------------------------
-- 이렇게 해서 정의어에 대해 알아보았는데
-- DDL : 사실상 신입한테 맡기지 않음!
-- 경력있는 DBA들이 테이블을 만들어서 데이터를C 만들거나R 수정하거나U 지우는D 정도만(CRUD) 하게끔 할 것
