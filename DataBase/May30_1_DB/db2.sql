...
--		varchar2(5 char) : 
--			한글/영어 5글자 => 최대 15byte
-- 	=> 문자형 자료형을 쓸 때은 용량에 char를 넣어서 쓰자! 
...
--		number(8, -1) 		=> ? 120 (s가 -1이고, 소수점 왼쪽 첫자리 3이 반올림)
...
insert into testNum vlaues(12345, 12345);
select * from testNum; -- 12345, 12000

-- float(5) 라고 하면, 5자리 10진수가 아닌, 5자리 2진수(ex:10110)을 의미
--	여기서 5는 부동 소수점 : 컴퓨터에서 실수를 표현하는 방식
--		실수를 정확히 2진수로 표현하기 어려운 문제를 해결하기 위한 방식
--	부동 소수점 숫자의 저장 방식은 특정 비트 수로 제한되어 있음
--		=> 실제 저장되는 값은 제한된 비트 수에 딸 반올림함
--	12345(10진수) = 11000000111001(2진수) => 여기서 float(5)는11000만 저장하고 나머지는 0처리
--	따라서 5자리 2진수 중에서 가장 가까운 값이 11000000000000(2진수)
--	이것을 다시 10진수로 변환하면
--		=> 11000000000000(2진수) = 12288(10진수)
--	그런데 변환하는 과정에서도 부동 소수점의 한계에 의 해 가장 근사치의 값인 12000 값이 도출
--		정밀도 제한, 반올림 오류, 2진수와 10진수의 변환 오류등
