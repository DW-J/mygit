...
--insert into person values('박길동', 3000, '남'); -- 오류 !
...
-- ex)
select * from person; -- => person 이라는 테이블로부터 모든(*) 데이터를 불러오겠다
select p_age, p_gender, p_name from person;alter
-- 순서만 바뀌었지 내용에는 변화가 없다 !

-- 5. 테이블 내의 중복된 튜플(행)은 허용되지 않는다.
