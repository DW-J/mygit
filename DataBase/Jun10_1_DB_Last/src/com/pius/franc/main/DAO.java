...
public class DAO {
	...
	//			: DAO의 작업결과를 표현하는 클래스 (Reservation / Restaruant)
	...
	// 	java,sql.Date => java.util.Date : 알아서 바꿔줌
		...
		public static ArrayList<Reservation> getAllReserve() {
			...
			try {
				...
				while (rs.next()) {
					...
					r.setName(rs.getNString("r_name"));
		...
		// 5. 매장찾기 (입력한 좌석 수 이상을 가지고 있는 매장, 지점명 오름차순 정렬)
...
//				return rsts;
			...
			try {
				...
				while (rs.next()) {
					rsvs.add(new Reservation(rs.getInt("r_no"), 
							rs.getString("r_name"), rs.getDate("r_time"), 
		...
		public static String updateReserve(Reservation rsv) {
			...
			try {
				...
				pstmt = con.prepareStatement(sql);
				pstmt.setNString(1, rsv.getPhone());
