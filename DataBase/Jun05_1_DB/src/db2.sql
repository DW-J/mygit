...
----------------------------------------------------------------------------------------------
-- 메뉴 데이터를 갯수를 나눠서 페이지로 보주고 싶음
--		어떻게 할까...?
-- 메뉴테이블에 m_no가 있음 (Primary key)
--		Sequence로 처리했음
--		그.런.데.말.입.니.다 sequence는 insert에 실패해도 올라감
--		메뉴 데이터 삭제도 가능!
--	=> m_no가 적확히 1, 2, 3, 4, ... 가 아님
-- 그럼 어떤 방법이 있을까...? 
----------------------------------------------------------------------------------------------
-- Rownum (가상필드) *************************
-- 		select을 할 때마다 자동으로 부여됨 (숫자)
--		* 이랑은 같이 사용할 수 없음
--		order by보다 먼저 rownum값이 먼저 부여
--		rownum값은 무조건 1번부터 조회해야 함!!

-- 메뉴 이름을 가나다 순으로 정렬해서 메뉴 전체 정보 조회
select * from jun05_menu order by m_name;

select rownum, * from jun05_menu order by m_name; -- 오류!! 문법 에러
...
select rownum, m_no, m_name, m_price, m_f_no 
	from jun05_menu 
	where rownum >= 1 and rownum <=3	-- 이렇게 하면 order by전에 rownum이 생성되어서
										-- sequence를 먹인 m_no와 같게 나와버림
										-- 우리가 의도한 바는 이름순으로 정렬을 하구
...
select rownum, m_no, m_name, m_price
	from (
		...
		--		뷰를 생성할 때 해당 컬럼을 삐고 생성함으로서 보안성에 좋음
		-- 2. 기존 테이블의 구조가 변경되어도 뷰의 규조를 변경하지 않아도 됨
	)
...
select *  -- rownum, rn, m_no, m_name, m_price로 대체 가능!!!
	from(
