...
--		- Round(숫자, m) : 숫자 반올리, m은 반올림 기준 자릿수 	
--												ex) ROUND(5.36, 1)
--		- POWER(숫자, n) : 숫자의 n제곱 값을 계싼 	ex) POWER(2, 3)
--		- TRUNC(숫자, m) : 숫자의 m번째 자리까지 남기고 값을 버림	 
--												ex) TRUNC(123.456, 2)
--												ex) TRUNC(123.456, -1)
--												ex) TRUNC(123.456, 0)
--		- MOD (m, n) : 숫자 m을 n으로 나눈 나머지 	ex) MOD(12, 10)
--		- SQRT(숫자) : 숫자의 제곱근 값을 계산 (숫자는 무조건 양수!)
--												ex) SQRT(9.0)
...
-- 2. 문자 함수
-- 		- CONCAT(s1, s2) : 두 문자열을 연결			ex) CONCAT('ABCD', 'EFGH')
--		- SIBSTR(s1, n, k) : 문자열의 n번째부터 k만큼의 길이를 반환
...
-- 오늘 날짜 기준 일 조회 (테이블 헤더에 '일' / 컬럼에 '203일' 이라고 나오게)
select to_char(sysdate, 'DD') || '일' 일 from dual;
-- 오늘 날짜 기준 시분 조회 (테이블 헤더에 '시', '분' / 컬럼에 '?시', '?분' 이라고 나오게)
