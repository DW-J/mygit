...
-- 데이터를 검색 or 조회할 수 이쓴ㄴ 명령어인 SELECT의 구조를 뜯어봤음
...
-- SEQUENCE의 특징은 테이블이랑 무관하고, 단점으로 INSERT에 실패하더라도,
--	SEQUENCE의 값은 무조건 올라간다는 것!!

-- 그 다음에 데이터를 검색할 수 있는 SELECT 쿼리문,
-- 그 형태는 select [distinct] [컬럼명], [컬럼명 as 별칭 || 컬럼명 별칭], [연산자], [통계함수]
--	from [테이블명]
--	where [조건식]
--	group by [그룹대삭]
--	having [함수 포함 조건]
...
-- 과자들 중 최고가?
select max(s_price) form JUN03_SNACK;
-- 제일 비싼 과자 이름, 제조사, 가격
select s_name, s_com, s_price 
	from JUN03_SNACK 
	where s_price = (
				select max(s_price)
				from JUN03_SNACK
...
insert into jun04_maker values('해태', '용산구', 1734);
insert into jun04_maker values('크라운', '한강대로', 1164);
insert into jun04_maker values('농심', '동작구', 5131);
insert into jun04_maker values('아노츠', '시드니', 4000);
---------------------------------------------------------------------------
...
select m_address 
	from jun04_maker
	where m_name = (		-- = 대신 in 을 쓰면 'single-row ~' 해결
		select s_com 
		from JUN03_SNACK 
		where s_price = (	-- 여기도 = 대신 in 써도 무방, 대신 =!, => 이런종류는 X
			select max(s_price) 
			from JUN03_SNACK
			)
	);
