...
//	가위바위보 게임 페이지를 만들 것
//	디자인은 알아서 ... (가위,바위,보 이미지 중에 하나를 누르면 게임 한판 진행
...
public class HomeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if (!request.getParameterNames().hasMoreElements()) {
			request.setAttribute("uh", "pepe.gif");
			request.setAttribute("ch", "pepe.gif");
		} else {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		RSPGameEngine rge = RSPGameEngine.getRge();
		RSPGameEngine.getRge().reset(request);
		request.setAttribute("uh", "pepe.gif");
		request.setAttribute("ch", "pepe.gif");
		request.getRequestDispatcher("rsp.jsp").forward(request, response);
